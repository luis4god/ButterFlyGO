...
visRecSetup.setupVisRec(visRecSetupParams, (err, data) => {
  if (err) {
    ...
    console.log('cites_from_space classifier_id: ' + data.classifier_id);
...
application.post("/uploadpic", function (req, result) {
  console.log('Recieving image: ' + req)
    ...
    form.parse(req, function (err, fields, files) {
        if (err) {
          ...
          console.log('uploadpic - file: ' + files)

            const filePath = JSON.parse(JSON.stringify(files));
            console.log('uploadpic - file: ' + filePath);
            console.log('uploadpic - filePath.myPhoto: ' + filePath.myPhoto);
            console.log('uploadpic - filePath.myPhoto.path: ' + filePath.myPhoto.path);
            const params = {
